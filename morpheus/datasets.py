...
class WSIRawRNADataset(WSIDataset):
    def __init__(self, dataframe, rna_dataframe, mode, data_dir, n_patches: int = 1024) -> None:
        super().__init__(dataframe, mode, data_dir, n_patches)
        self.rna_dataframe = rna_dataframe
        
    def _prepare_rna(self, submitter_id):
        patient_df = self.rna_dataframe[self.rna_dataframe.index == submitter_id]
        rna = torch.from_numpy(patient_df.values).float().squeeze(0)
        return rna
    ...
    def __getitem__(self, idx):
        ...
        wsi_embeddings = self._get_wsi_embedding(idx)
        rna = self._prepare_rna(submitter_id)
        return wsi_embeddings, rna


class WSIRawOmicsDataset(WSIRawRNADataset):
    def __init__(self, dataframe, rna_dataframe, dnam_dataframe, cnv_dataframe, mode, data_dir, n_patches: int = 1024) -> None:
        super().__init__(dataframe, rna_dataframe, mode, data_dir, n_patches)
        self.dnam_dataframe = dnam_dataframe
        self.cnv_dataframe = cnv_dataframe
        
    def _prepare_dnam(self, submitter_id):
        patient_df = self.dnam_dataframe[self.dnam_dataframe.index == submitter_id]
        dnam = torch.from_numpy(patient_df.values).float().squeeze(0)
        return dnam
    
    def _prepare_cnv(self, submitter_id):
        patient_df = self.cnv_dataframe[self.cnv_dataframe.index == submitter_id]
        cnv = torch.from_numpy(patient_df.values).float().squeeze(0)
        return cnv
