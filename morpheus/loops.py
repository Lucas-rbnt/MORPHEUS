...
def loop_classification(model, omics_modalities, train_dataloader, val_dataloader, optimizer, lr_scheduler, epochs, device, wandb_logging, mapping):
    if len(mapping) != 2:
        ...
        if epoch == epochs - 1:
            ...
            with torch.no_grad():
                for _, inputs in enumerate(tqdm.tqdm(val_dataloader, desc="Validation...")):
                    ...
                    if len(mapping) != 2:
                        loss = loss_fn(outputs, label)
                        predictions.append(outputs.softmax(-1).cpu().numpy())
                    else: 
                        loss = loss_fn(outputs.squeeze(-1), label.float())
