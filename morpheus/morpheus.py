...
class MorpheusProto(nn.Module):
    ...
    def forward(self, wsi, omics, omics_masks=None):
        ...
        if omics_masks is None:
            ...
            # round up task masking
            mask_all = torch.ones_like(mask_all)
            mask_all[:, :wsi_tokens.shape[1]+number_of_tokens_to_keep] = 0 
            mask_all = torch.gather(mask_all, dim=1, index=ids_restore) # unshuffle to get the binary mask
            
            n_tokens_per_omics = [omics_masks[modality].shape[1] for modality in self.omics_modalities]
            omics_masks = torch.split(mask_all[:, wsi_tokens.shape[1]:], n_tokens_per_omics, dim=1)
            omics_masks = OrderedDict((modality, omics_masks[i]) for i, modality in enumerate(self.omics_modalities))
