...
def get_classification_metadata(classification_task):
    ...
    if classification_task == "atrx":
        metadata = pd.read_csv("datasets_csv/metadata/tcga_gbmlgg.csv")
        metadata = metadata[metadata.split == "fine-tune"]
        metadata = metadata[metadata["ATRX"].notna()]
        label_col = "ATRX"
    
    if classification_task == "1p19q":
        metadata = pd.read_csv("datasets_csv/metadata/tcga_gbmlgg.csv")
        metadata = metadata[metadata.split == "fine-tune"]
        metadata = metadata[metadata["X1p19q"].notna()]
        label_col = "X1p19q"
    ...
    if classification_task == 'pancan':
        metadata = pd.read_csv("datasets_csv/metadata/tcga_pancan.csv")
        metadata = metadata[metadata.split == "fine-tune"]
        metadata = metadata[metadata["project_id"].notna()]
        label_col = "project_id"


    if classification_task == "egfr":
        metadata = pd.read_csv("data/files/clinical_cptac.csv")
        metadata = metadata[metadata.EGFR.notna()]
        metadata = metadata[metadata["EGFR"].notna()]
        label_col = "EGFR"

    if classification_task == "tp53":
        metadata = pd.read_csv("data/files/clinical_cptac.csv")
        metadata = metadata[metadata.TP53.notna()]
        metadata = metadata[metadata["TP53"].notna()]
        label_col = "TP53"

    if classification_task == "mgmt":
        metadata = pd.read_csv("datasets_csv/metadata/tcga_gbmlgg.csv")
        metadata = metadata[metadata.split == "fine-tune"]
        metadata = metadata[metadata["MGMT"].notna()]
        label_col = "MGMT"

    if classification_task == "response":
        metadata = pd.read_csv("datasets_csv/metadata/tcga_gbmlgg.csv")
        metadata = metadata[metadata.split == "fine-tune"]
        metadata = metadata[metadata["response"].notna()]
        label_col = "response"
...
def compute_clf_metrics(labels, predictions):
    if len(np.unique(labels)) > 2:
        ...
        balanced_accuracy = balanced_accuracy_score(labels, predictions.argmax(axis=1))
    else:
