"""
Inspired from https://github.com/mahmoodlab/SurvPath/blob/main/utils/loss_func.py
The get_survival_labels is also copy/paste from their dataset source code.
"""
...
def get_survival_labels(metadata, time_col, censorship_col, n_bins, eps=1e-6):
    uncensored_df = metadata[metadata[censorship_col] < 1]
    disc_labels, q_bins = pd.qcut(uncensored_df[time_col], q=n_bins, retbins=True, labels=False)
    q_bins[-1] = metadata[time_col].max() + eps
    q_bins[0] = metadata[time_col].min() - eps
    
    disc_labels, _ = pd.cut(metadata[time_col], bins=q_bins, retbins=True, labels=False, right=False, include_lowest=True)
    metadata.insert(2, 'label', disc_labels.values.astype(int))
