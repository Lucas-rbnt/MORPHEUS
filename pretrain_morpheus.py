...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_layers_decoders', type=int, default=1)
    parser.add_argument("--batch_size", type=int, default=128, help="Batch size for training")
    parser.add_argument("--wsi_type", type=str, default="proto", help="Type of WSI model to use")
    ...
    parser.add_argument("--num_proto", type=int, default=16, help="Number of prototypes for the model")
    parser.add_argument("--entity", type=str, default="luktenib", help="Entity for wandb logging")
    parser.add_argument("--project", type=str, default="morpheus-pretrain-end", help="Project for wandb logging")
    ...
    # parser.add_argument("--use_modalities_embeddings", action='store_true', help="Whether to use modalities embeddings")
    ...
    if wandb_logging:
        import wandb
        run = wandb.init(project=args.project, entity=args.entity, reinit=True, config=vars(args), name=f"morpheus{args.wsi_type.upper()}_{str(int(args.masking_ratio * 100))}_embed-mod={args.use_modalities_embeddings}_{args.project_id}_mod={len(args.omics_modalities)}_lay={args.num_layers}")
    ...
    base_path = f"pretrained_models/morpheus{args.wsi_type.upper()}_{str(int(args.masking_ratio * 100))}_embed-mod={args.use_modalities_embeddings}_{args.project_id}_mod={len(args.omics_modalities)}_layers={args.num_layers}_epochs="
